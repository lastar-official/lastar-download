...
for(const route of routes){const html=await readFile(resolve(root,route),'utf8');assert.equal((html.match(/<h1[ >]/g)||[]).length,1,`${route}: one h1`);for(const required of ['name="description"','rel="canonical"','property="og:image"','name="theme-color"','rel="apple-touch-icon"','class="skip"'])assert.ok(html.includes(required),`${route}: missing ${required}`);assert.ok(!/mailto:|support@|google-analytics|googletagmanager|hotjar|<iframe|javascript:|innerHTML|eval\(/i.test(html),`${route}: forbidden integration`);for(const match of html.matchAll(/\b(href|src)="([^"]+)"/g)){const url=match[2];if(url.startsWith('#')){assert.ok(html.includes(`id="${url.slice(1)}"`),`${route}: missing anchor ${url}`);continue;}if(/^https:/.test(url)){const host=new URL(url).hostname;assert.ok(['lastar.me','github.com'].includes(host),`${route}: unknown external host`);continue;}assert.ok(url.startsWith('/'),`${route}: noncanonical relative link ${url}`);let path=resolve(root,'.'+url.split('#')[0]);try{if((await stat(path)).isDirectory())path=resolve(path,'index.html');await stat(path);}catch{assert.fail(`${route}: missing local asset/link ${url}`);}links++;}if(!meta.public_verified)assert.ok(!/href="https:\/\/github.com\/[^\"]+\.apk"/.test(html),`${route}: unverified binary link`);}
...
for(const route of routes){
 ...
 assert.ok(html.includes('data-owner-support')&&html.includes('由个人运营，尚未注册公司')&&html.includes('APP 备案尚未办理')&&html.includes('具体城市尚未补充'),`${route}: owner facts are not invented`);
 assert.ok(html.includes('hello@lastar.me')&&html.includes('验收尚未完成，暂不列为已验证可用渠道')&&html.includes('48 小时内')&&html.includes('不声称已具备官方域名回信能力'),`${route}: support remains unverified`);
}
...
for(const [name,html]of [['home',home],['download',download]]){
 assert.ok(html.includes('最多 10 名')&&html.includes('已知成年测试者')&&html.includes('中国大陆')&&html.includes('免费封闭技术内测'),`${name}: exact owner-authorized first stage`);
 assert.ok(html.includes('NO-GO')&&html.includes('暂不开测、不公开招募'),`${name}: no premature GO`);
 assert.ok(!html.includes('请关注「LaStar 拉星」的招募信息。')&&!html.includes('关注「LaStar 拉星」，了解内测招募'),`${name}: no public WeChat recruitment`);
}
assert.ok(download.includes('data-checksum-correction')&&download.includes('APK、正式签名与版本未替换'), 'Corrected checksum is distinct from unchanged APK');
